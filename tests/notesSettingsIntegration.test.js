...
test('Settings is fixed-height and shares Save across S3, Notes, and local LLM tabs', async () => {
  ...
  assert.match(persistentReload, /event\.notesDelta[\s\S]*?applyNotesPageDelta\(event\.notesDelta, event\.persistentApplyId\)/);
  assert.match(persistentReload, /event\.persistentApplyId[\s\S]*?reloadNotesPage\(event\.persistentApplyId\)/);
  assert.match(persistentReload, /event\.hostsChanged[\s\S]*?Promise\.all\(\[loadHosts\(\), notesUpdate\]\)/);
  assert.match(notesPage, /async applyPersistentDelta\(delta, persistentApplyId\)[\s\S]*?removedNoteIds[\s\S]*?upsertedNotes/);
