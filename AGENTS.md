...
- `src/renderer/kubernetesWorkspace.ts`: reusable multi-tab bottom Logs/Shell workspace with compact target-only, type-accessible amber Logs/blue Shell tab visuals, integrated SVG close controls, bounded pointer/keyboard height resizing, a bounded pre-bind renderer terminal-output bridge, and exact tab/session lifecycle-disposal ownership.
