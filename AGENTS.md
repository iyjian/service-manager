...
- `src/main/kubernetes/kubernetesClient.ts`: main-process Kubernetes REST, CRD discovery, UID-checked Pod→ReplicaSet→Deployment log-target resolution, Watch, log, exec, port-forward, and on-demand relation adapter.
...
- `src/main/kubernetes/podInteractions.ts`: 2,000-line bounded single-Pod/Deployment aggregate logs with multi-stream generation fencing, terminal shell fallback/session ownership, and ten-forward lifecycle.
...
- `src/renderer/kubernetesPage.ts`: full-width Context/Namespace controls with an unclipped Namespace popup, category lists, right-side overlay drawers with aligned Labels and fully expanded Env rows, text-safe browser-YAML rendering, Events, on-demand relations, bottom workspace, and forward UI.
...
- `src/renderer/kubernetesWorkspace.ts`: reusable multi-tab bottom Logs/Shell workspace with Deployment scope switches, post-layout paused-scroll restoration, compact target-only type-accessible tab visuals, integrated SVG close controls, bounded pointer/keyboard height resizing, a bounded pre-bind renderer terminal-output bridge, and exact tab/session lifecycle-disposal ownership.
...
- Kubernetes alone uses the available application width as the full-width Kubernetes browser rather than the shared centered-content width. Keep compact, label-free Context and Namespace selectors on one toolbar row with non-wrapping category and resource controls; the Namespace popup must remain unclipped while category, resource, and secondary control strips retain their own horizontal scrolling. There is no UI Cluster category. Categories are Workloads (Pods, Deployments, StatefulSets), Network (Services, Ingresses), Configuration (ConfigMaps, Secrets), Storage (PVC), and dynamically discovered Custom Resources. Discover CRDs only when that category is active, through read-only ApiextensionsV1 API; choosing Custom Resources must immediately show its Group/Version/Kind select without a redundant resource-type button.
- Keep Context and Namespace controls on one row. The Namespace dropdown lists active-Context Namespaces through a read-only paged request and provides checkbox multi-select between selected Namespaces and the mutually-exclusive `All` menu item, with no manual Namespace entry. A real Context change must invalidate/reload the Namespace list and reset scope to All Namespaces. Keep the unclipped Namespace menu open during multi-select and close it after an outside pointer press. All Namespaces uses one active scope-wide Watch for the current resource type only; Nodes and Namespaces remain cluster-scoped.
...
- Kubernetes alone uses the available application width in the existing white content-container style: compact label-free Context/Namespace selectors, TLS/disconnect state, selected Namespace scope, non-wrapping category/resource controls, and virtual resource tables; no UI Cluster category is visible. The Namespace popup remains unclipped, while other constrained control strips scroll internally. Resource details are right-side overlay drawers that preserve the originating list's loaded pages, filters, sort, active Watch, and scroll state beneath. Labels align keys left and values right; expanded Env shows fully wrapped bounded key/value rows without source/reference badges and scrolls with the drawer. YAML is an icon action, and Events are read-only/on-demand. At 1230×820, drawer/workspace scrolling stays internal; smaller windows keep primary controls visible and move overflow into the active drawer and bottom workspace. Empty Port Forwards UI stays hidden, and active forwards plus the bottom Logs/Shell workspace remain viewport-contained layers.
...
- Kubernetes container actions sit next to the container name and open/focus multiple closable bottom Logs/Shell workspace tabs keyed by namespace/Pod/container/type. Their 24px-high visible captions are target-only (`namespace/pod · container`) with a 140px caption cap, their type remains accessible, Logs tabs/icons are amber, Shell tabs/icons are blue, and close uses an integrated SVG. Do not repeat the target title inside the selected Logs/Shell pane. Keep search/actions/status at 24px, and expose the 6px pointer/keyboard resize separator with a 120px minimum and 80%-of-page maximum; refit an active xterm without recreating it. Drawer close preserves tabs; individual close/final and Context/page/disconnect/shutdown cleanup dispose exact local views/sessions and block late output from reviving closed tabs. Leaving the Kubernetes tab stops Watches, logs, and terminal sessions, while explicit port forwards remain until their Context lifecycle ends.
...
- Kubeconfig bytes, absolute source paths, stable-ID source mappings, tokens, client certificates/keys, client transport handles, and terminal/port-forward credentials stay in the main process and never cross renderer IPC. Terminal output may be transiently relayed but must never be persisted to main-process state, disk, settings, or logs; retained xterm views and the bounded pre-bind bridge are renderer-memory-only.
