...
- `src/main/kubernetes/kubernetesClient.ts`: main-process Kubernetes REST with a read-only Version reachability probe, CRD discovery, UID-checked Pod→ReplicaSet→Deployment log-target resolution, Watch, log, exec, port-forward, and on-demand relation adapter.
