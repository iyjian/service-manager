...
- `src/main/kubernetes/kubernetesClient.ts`: main-process Kubernetes REST with a read-only Version reachability probe, CRD discovery, UID-checked Pod→ReplicaSet→Deployment log-target resolution, Watch, log, UTF-8/TTY-bootstrapped exec with streaming output decoding, port-forward, and on-demand relation adapter.
...
- `src/main/kubernetes/podInteractions.ts`: 2,000-line bounded single-Pod/Deployment aggregate logs with multi-stream generation fencing, terminal shell fallback/session ownership and first-output readiness gating, and ten-forward lifecycle.
