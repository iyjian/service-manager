import type {
  ...
  KubernetesCustomResourcePrinterColumnType,
} from '../../shared/types';
...
const DEFAULT_FORMAT_LENGTH = 512;
const FORBIDDEN_KEYS = new Set(['__proto__', 'prototype', 'constructor']);
const PRINTER_COLUMN_TYPES = new Set<KubernetesCustomResourcePrinterColumnType>([
...
/**
 * Parses the non-executable JSONPath subset used by Kubernetes printer columns.
 * It supports fields, quoted keys, indexes, wildcards, and primitive equality
 * filters without evaluating scripts or prototype properties.
 */
...
function applyPathStep(nodes: unknown[], step: PathStep, budget: EvaluationBudget): unknown[] {
  ...
  for (const node of nodes) {
    ...
    if (step.kind === 'field') {
      ...
      for (const child of node) {
        if (!visit(budget)) break;
        const actual = readFilterPath(child, step.path);
...
export function readKubernetesPrinterColumnValue(source: unknown, jsonPath: string): unknown {
  ...
  // Kubernetes table conversion uses the first JSONPath result for a cell.
  return nodes[0];
}
...
export function formatKubernetesPrinterColumnValue(
  ...
  const maxLength = Math.max(16, Math.min(options.maxLength ?? DEFAULT_FORMAT_LENGTH, 16_384));
...
/**
 * Binds only the CRD columns that can fit in the fixed eight-column list.
 * sourceIndex keeps renderer keys aligned after priority ordering.
 */
export function selectKubernetesPrinterColumnsForList(
  value: KubernetesCustomResourcePrinterColumn[],
  scope: 'namespaced' | 'cluster',
): KubernetesCustomResourcePrinterColumn[] {
  const available = scope === 'namespaced' ? 5 : 6;
  return value
    .map((column, sourceIndex) => ({ column, sourceIndex }))
    .filter(({ column }) => !['age', 'name', 'namespace'].includes(column.name.toLocaleLowerCase()))
    .sort((left, right) => left.column.priority - right.column.priority || left.sourceIndex - right.sourceIndex)
    .slice(0, available)
    .map(({ column, sourceIndex }) => ({ ...column, sourceIndex }));
}

/** Keeps only display metadata needed by the read-only list/detail renderer. */
export function normalizeKubernetesPrinterColumns(value: unknown): KubernetesCustomResourcePrinterColumn[] {
  if (!Array.isArray(value)) return [];
  const result: KubernetesCustomResourcePrinterColumn[] = [];
  const names = new Set<string>();
  for (const entry of value) {
    if (result.length >= MAX_PRINTER_COLUMNS) break;
    const candidate = record(entry);
    const name = typeof candidate?.name === 'string' ? candidate.name.trim() : '';
    const type = candidate?.type;
    const jsonPath = typeof candidate?.jsonPath === 'string' ? candidate.jsonPath.trim() : '';
    const priority = candidate?.priority === undefined ? 0 : candidate.priority;
    const normalizedName = name.toLocaleLowerCase();
    if (!name || name.length > MAX_COLUMN_NAME_LENGTH || normalizedName === 'age' || names.has(normalizedName)
      || !PRINTER_COLUMN_TYPES.has(type as KubernetesCustomResourcePrinterColumnType)
      || !jsonPath || !parsePath(jsonPath)
      || !Number.isInteger(priority) || (priority as number) < 0 || (priority as number) > 1_000_000) {
      continue;
    }
    names.add(normalizedName);
    result.push({
      name,
      type: type as KubernetesCustomResourcePrinterColumnType,
      jsonPath,
