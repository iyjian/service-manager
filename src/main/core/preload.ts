...
const api: ServiceApi = {
  openSshTerminal: (hostId, id) => ipcRenderer.invoke('ssh-terminal:open', { hostId, id }),
  openLocalTerminal: (id) => ipcRenderer.invoke('local-terminal:open', { id }),
  writeLocalTerminal: (id, data) => ipcRenderer.invoke('local-terminal:write', { id, data }),
  resizeLocalTerminal: (id, cols, rows) => ipcRenderer.invoke('local-terminal:resize', { id, cols, rows }),
  closeLocalTerminal: (id) => ipcRenderer.invoke('local-terminal:close', { id }),
  acknowledgeLocalTerminalOutput: (id, characters) => ipcRenderer.invoke('local-terminal:acknowledge', { id, characters }),
  onLocalTerminalChanged: (listener) => subscribe('local-terminal:state', listener),
  onLocalTerminalOutput: (listener) => subscribe('local-terminal:output', listener),
  writeSshTerminal: (id, data) => ipcRenderer.invoke('ssh-terminal:write', { id, data }),
  resizeSshTerminal: (id, cols, rows) => ipcRenderer.invoke('ssh-terminal:resize', { id, cols, rows }),
  closeSshTerminal: (id) => ipcRenderer.invoke('ssh-terminal:close', { id }),
  acknowledgeSshTerminalOutput: (id, characters) => ipcRenderer.invoke('ssh-terminal:acknowledge', { id, characters }),
  onSshTerminalChanged: (listener) => subscribe('ssh-terminal:state', listener),
  onSshTerminalOutput: (listener) => subscribe('ssh-terminal:output', listener),
  ...
  markChangelogSeen: () => ipcRenderer.invoke('changelog:mark-seen'),
