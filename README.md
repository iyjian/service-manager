...
- `src/renderer/notesPage.ts`: full-width split-pane CodeMirror 6/Tiptap Notes orchestration with indexed input-debounced search, cached breadcrumbs, row-local name/save updates, a drag-and-drop hierarchy, persisted accessible tree resizing, optimistic subtree deletion with lightweight authoritative previews, request-generation draft fencing, configurable theme/font sizing, S3-image flow, and debounced live save
