...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - resource details are full-page, read-only Overview, YAML, and on-demand Events views presented as a single-screen workbench at the default 1230×820 window size. The detail header's Port Forward action replaces Copy. Overview metadata, related resources, YAML, Events, and Logs use bounded internal scrolling when space is constrained; the page itself does not grow with detail content. Deployment/StatefulSet related Pods and Service Endpoints/EndpointSlices load only after expansion and never add a Watch
    - Pod logs open automatically for the active Pod/container, load 500 initial lines, follow by default, and keep a 2,000-line log cap per viewer. While following, Follow shows a pause icon; when paused it shows a play icon. A manual upward scroll does not detach follow permanently: the next log update returns the viewport to the bottom, while Pause preserves position and Resume immediately returns to the bottom. The toolbar keeps Logs, Terminal, search, Follow, Clear, and Container on one internally scrollable row and exposes no manual older-page action. Log streams and terminal sessions close when leaving the Kubernetes page, switching/disconnecting the Context, or exiting the application
    - selecting the Terminal tab opens or focuses the matching Pod session through the global terminal drawer; there is no Open Terminal button. The drawer can hold multiple sessions and tries `/bin/sh`, then `ash`, then `bash`; each newly opened terminal scrolls into view and is focused for immediate input, while terminal input preserves spaces, Enter, and xterm control sequences exactly within the bounded IPC payload
    - the header Port Forward dialog uses Pod regular containers and restartable native sidecar init containers to discover declared TCP ports, and uses a Service's `spec.ports[].port`; declared ports are not proof that a listener is running. With zero declarations, manual Remote Port entry remains available; with one declaration, the editable Remote Port prefills; with multiple declarations, the user selects a declared port or enters a Remote Port manually. Pod and Service forwards can use an automatic or selected local port; at most ten active port forwards are allowed. Forwards survive a detail-page close, but stop on Context switch, disconnect, or application exit
...
- `src/renderer/kubernetesPage.ts`: Kubernetes Context/scope controls, category lists, full-page details, text-safe browser-YAML rendering, on-demand relations, logs, and forwards UI
