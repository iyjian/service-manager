...
16. The bottom navigation Settings action provides automatic, cloud-authoritative synchronization through a MinIO-compatible S3 bucket:
    ...
    - all clients use the incompatible v3 object layout. The canonical `service-manager/v3/head.json` references one immutable encrypted `service-manager/v3/manifests/<revision>.json` manifest; that manifest carries the shared Hosts and Proxy sections, Note tombstones, and a validated immutable-object reference for every active Note instead of embedding every Note body. Each reference binds the stable Note ID to an opaque object ID plus ciphertext and plaintext-content digests. The v1 and v2 prefixes are never read, migrated, promoted, or deleted
    ...
    - Hosts and Proxy are merged as complete sections: a one-sided change is retained, while a true same-section divergence follows the cloud. Before applying a cloud-winning Hosts or Proxy conflict, the app writes an encrypted local recovery revision under `<userData>/s3-sync-recovery` and retains the newest 20 files; recovery files are not applied automatically
    ...
    - AK and SK are protected at rest with Electron `safeStorage`; Linux `basic_text` storage is rejected rather than persisting credentials insecurely. Ordinary settings reads disclose only whether credentials exist. Opening Settings uses a dedicated narrow IPC to refill saved values into password-masked inputs; either eye may reveal only its selected field and immediately re-masks the other. Clearing credentials may retain only a `safeStorage`-protected one-way SK fingerprint so restoring the same encryption key keeps the established merge base; it is never returned through renderer IPC. AK or Region changes keep that base, while an Endpoint, Bucket, or verified SK change resets it
    - the immutable manifest and every immutable Note object are separately encrypted with AES-256-GCM using HKDF-derived keys based on the Secret Access Key, and their references are digest-validated. AWS SigV4 signs every S3 request. Clients sharing a sync target must therefore use the same SK to decrypt one another's data. Plaintext or protected S3 credential material never enters cloud objects, runtime logs, diagnostics, or error messages
...
- `src/main/s3SyncV3.ts`: v3 head/manifest/per-Note object contracts, independent AES-GCM envelopes, digest-bound references, SigV4 object access, ETag head compare-and-swap, and abortable request bounds
...
- `src/main/s3Sync.ts`: `safeStorage`-protected MinIO/S3 settings and migration, automatic v3 single-flight reconciliation, unchanged Note-reference reuse, independently serialized durable offline/pending intent, encrypted conflict recovery, and bounded shutdown ownership
