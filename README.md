...
- `src/renderer/notesRichTextEditor.ts`: isolated Tiptap adapter with a selection-only Novel-style formatter and slash block menu, checked task lists, canonical-document serialization, absolute-HTTP(S)-only inert link editing, safe S3-image paste/drop and node views, revocable blob-URL ownership, and disabled remote-image ingestion
