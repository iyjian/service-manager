...
15. Notes provides a compact local snippet library:
    - the viewport-contained two-column page uses the full available application width. Its Notion-style Note tree has a `280px` default sidebar that can be dragged or adjusted by keyboard between `240px` and `520px`; that exact device-local width survives window resizing and application restarts while only the editor column changes width. The tree owns inline root/child creation, collapse controls, subtree deletion, keyboard navigation, and before/inside/after drag-and-drop; both constrained areas scroll internally at smaller window sizes. Rich Text keeps compact horizontal writing padding—24px at ordinary window widths and 12px below the narrow breakpoint—and compensates for its proportional-font metrics so its first visible text line aligns with Markdown while retaining bottom breathing room. Expanded nodes are device-local, while hierarchy and sibling order are durable shared data
    ...
    - Settings > Notes also provides a read-only Trilium ETAPI import. Enter the Trilium Endpoint and ETAPI Token for that import only: neither is saved as importer configuration, the Token is never persisted, remains password-masked unless its eye action is used, and is cleared on success, cancellation, or dialog close. Canonical imported Note content can retain source URL text and resolved Trilium links just like any other Note content. The importer reports discovery, fetch, Rich Text conversion, and apply progress in the fixed-height Settings dialog; Cancel is available until the final atomic apply begins. An HTTP Endpoint is allowed with a visible transport-security warning, while ETAPI redirects are always rejected
    - the importer walks the user hierarchy from Trilium's root through read-only Note, Branch, and content requests, skips root system trees whose Note ID starts with `_`, and never creates, edits, or deletes anything in Trilium. A canonical Endpoint plus Trilium Note ID produces the stable local Note ID, while a bounded source-version hash tracks the importer version and source title/type/MIME/blob/modified metadata. Reimporting an unchanged source version preserves the complete local Note, including local edits. When that source version changes, the imported Name, Language, Content, updated timestamp, and importer markers are refreshed while the original local creation time and unrelated internal Tags are retained. A Trilium deletion is not mirrored: previously imported local Notes absent from a later traversal remain local
    - Trilium clone placements are deduplicated by remote Note ID and use the first reachable breadth-first tree placement. HTML Notes are sanitized and converted through the same Tiptap extension schema, then persisted only after shared canonical Rich Text validation. Trilium image/file Notes, protected or oversized content, and unsupported binary types become explicit Markdown placeholders; images embedded inside imported HTML become text placeholders rather than being fetched or uploaded. Import preparation is bounded to 10,000 Notes, tree depth 32, 50,000 Branches, 4 MiB per ETAPI response, and 50 MiB transferred in total, with eight requests in flight and a 15-second per-request timeout
...
- `src/main/triliumImport.ts`: bounded read-only Trilium ETAPI hierarchy/content traversal, stable source identity/versioning, clone deduplication, placeholder generation, and deterministic local Note/tree merge planning
...
- `src/renderer/notesPage.ts`: full-width split-pane CodeMirror 6/Tiptap Notes orchestration with a searchable drag-and-drop hierarchy, persisted accessible tree resizing, subtree actions, request-generation draft fencing, configurable theme/font sizing, S3-image flow, and debounced live save
