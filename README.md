...
- `src/main/main.ts`: Electron app/window/menu wiring and IPC orchestration after runtime-data profile selection
...
- `src/main/s3Sync.ts`: `safeStorage`-protected MinIO/S3 credentials and independent Sync Encryption Key, v4-only automatic single-flight reconciliation with privacy-safe phase/item progress, same-key Note/tree-reference reuse, independently serialized durable offline/pending intent, encrypted conflict recovery, key rotation, and bounded shutdown ownership
...
- `src/renderer/notesPage.ts`: full-width split-pane CodeMirror 6/Tiptap Notes orchestration with indexed input-debounced search, cached breadcrumbs, row-local name/save updates, a drag-and-drop hierarchy, persisted accessible tree resizing, optimistic subtree deletion with lightweight authoritative previews, request-generation draft fencing, persistent-apply inert leases, deferred editor capture, configurable theme/font sizing, S3-image flow, and debounced live save
- `src/renderer/notesRichTextEditor.ts`: isolated Tiptap adapter with a selection-only Novel-style formatter, keyboard-contained slash block menu, official resizable TableKit, checked task lists, save-time canonical-document serialization, animation-frame-coalesced editor chrome, absolute-HTTP(S)-only inert link editing, safe S3-image paste/drop and node views, revocable blob-URL ownership, and disabled remote-image ingestion
