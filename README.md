...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - Pod logs open automatically for the active Pod/container, load 500 initial lines, follow by default, and keep a 2,000-line log cap per viewer. Log streams and terminal sessions close when leaving the Kubernetes page, switching/disconnecting the Context, or exiting the application
    - the global terminal drawer can hold multiple Pod sessions and tries `/bin/sh`, then `ash`, then `bash`. Each newly opened terminal scrolls into view and is focused for immediate input. Pod and Service forwards can use an automatic or selected local port; at most ten active port forwards are allowed. Forwards survive a detail-page close, but stop on Context switch, disconnect, or application exit
