...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - the Namespace dropdown loads the cluster's Namespaces through the read-only API and supports checkbox multi-select between selected Namespaces and the mutually-exclusive All Namespaces shortcut; there is no manual Namespace entry. The unclipped Namespace menu stays open during multi-select and closes after an outside pointer press. Nodes and Namespaces remain cluster-scoped regardless of this selection
...
- `src/renderer/kubernetesWorkspace.ts`: reusable multi-tab bottom Logs/Shell workspace with target-only, type-accessible amber Logs/blue Shell tab visuals, integrated SVG close controls, a bounded pre-bind terminal-output bridge, and exact tab/session lifecycle disposal ownership
