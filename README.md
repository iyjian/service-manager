...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - Overview metadata uses compact single-line label/value rows with long values truncated instead of wrapped
    - resource details are full-page, read-only Overview, YAML, and on-demand Events views presented as a single-screen workbench at the default 1230×820 window size. Overview metadata, related resources, YAML, Events, and Logs use bounded internal scrolling when space is constrained; the page itself does not grow with detail content. Deployment/StatefulSet related Pods and Service Endpoints/EndpointSlices load only after expansion and never add a Watch
    - Pod logs open automatically for the active Pod/container, load 500 initial lines, follow by default, and keep a 2,000-line log cap per viewer. While Follow is enabled, every new log update returns the viewport to the latest line even after a manual upward scroll; Pause preserves position and Resume immediately returns to the bottom. The toolbar keeps search, Follow, and Clear on one row and exposes no manual older-page action. Log streams and terminal sessions close when leaving the Kubernetes page, switching/disconnecting the Context, or exiting the application
    - the global terminal drawer can hold multiple Pod sessions and tries `/bin/sh`, then `ash`, then `bash`. Each newly opened terminal scrolls into view and is focused for immediate input; terminal input preserves spaces, Enter, and xterm control sequences exactly within the bounded IPC payload. Pod and Service forwards can use an automatic or selected local port; at most ten active port forwards are allowed. Forwards survive a detail-page close, but stop on Context switch, disconnect, or application exit
