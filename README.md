...
15. Notes provides a compact local snippet library:
    - the viewport-contained two-column page uses the full available application width, keeps a responsive `240–360px` searchable name list on the left, and lets the selected editor consume the remaining width; both constrained areas scroll internally at smaller window sizes
    - search ranks exact, prefix, and partial Name matches ahead of lower-priority Tag, Language, and full-content matches
    - each note has a Name, comma-separated Tags, Language, and Content editor. Markdown remains the default; Markdown, Bash, JavaScript, TypeScript, SQL, JSON, and YAML use the CodeMirror 6 editor with parser-backed dark-theme highlighting, while Plain Text remains unstyled. Markdown fenced blocks also highlight Bash, JavaScript, TypeScript, SQL, JSON, and YAML. The optional Rich Text language uses a local Tiptap editor and persists a validated canonical Tiptap JSON document instead of HTML
    - switching a non-empty note between Rich Text and a CodeMirror language requires confirmation because the conversion is intentionally text-oriented and can lose formatting. Rich Text images require a configured S3 target and credentials; without them the image action shows a configuration prompt. Uploads accept only PNG, JPEG, and WebP up to 10 MiB, with each dimension limited to 8192px and the decoded image limited to 40 million pixels
    - edits save automatically after a short debounce, note switches and page changes flush pending edits, and window/application close performs a bounded renderer-to-main flush handshake before the final close; every Note ID has one independent versioned `<userData>/notes/<sha256(note.id)>.json` file containing either the active Note or its deletion tombstone, so one edit rewrites only one file. Service Manager acquires Electron's single-instance lock before initializing persistent stores so two app processes cannot race the Notes directory transaction. The directory and files use private permissions where supported; the retired aggregate `<userData>/notes.json` is intentionally neither read nor migrated
    ...
    - dynamic content is rendered only through CodeMirror/Tiptap state, form values, or text nodes; stored Rich Text is normalized through the shared safe-node model, and fetched images are exposed to Tiptap only through short-lived blob URLs. The bounded left list remains independently scrollable with large note collections
    - Notes typography is packaged with the application: Noto Sans SC Variable renders the Notes UI and mixed Chinese/Latin text, while JetBrains Mono Variable renders CodeMirror code. Chinese editor glyphs fall back to the bundled Noto family, so Notes does not depend on fonts installed by the operating system or fetch fonts at runtime. The Notes Settings tab adjusts the editor from 12–24px; 14px is the default, and the device-local choice is stored in the versioned private `<userData>/ui-preferences.json`
16. The bottom navigation Settings action provides automatic, cloud-authoritative synchronization through a MinIO-compatible S3 bucket:
    - Settings uses compact `S3` and `Notes` tabs. One shared `Save` action persists both tabs and closes the dialog only after every requested write succeeds. The S3 tab owns `Test` and `Sync Now`: `Test` signs a bounded read of the canonical head with the current unsaved Endpoint/Bucket/Region/AK/SK draft without saving settings or starting reconciliation, while `Sync Now` saves the S3 draft and runs the ordinary single-flight reconciler
    ...
    - all clients use the incompatible v3 object layout. The canonical `service-manager/v3/head.json` references one immutable encrypted `service-manager/v3/manifests/<revision>.json` manifest; that manifest carries the shared Hosts and Proxy sections, Note tombstones, and a validated immutable-object reference for every active Note instead of embedding every Note body. The head, encrypted envelope, and each Note reference bind the Sync Encryption Key identity; each reference also binds the stable Note ID to an opaque object ID plus ciphertext and plaintext-content digests. Mixed-key manifests are rejected. The v1 and v2 prefixes are never read, migrated, promoted, or deleted
    - each active Note version is encrypted independently and uploaded as its own immutable `service-manager/v3/notes/<opaque-object-id>.json` object; stable Note IDs and names never enter object paths. A new manifest reuses unchanged Note references, so routine reconciliation uploads and downloads only changed Note objects rather than transferring every Note again. Deletion removes the active reference and retains its tombstone in the manifest
    - a Rich Text image is a separate private immutable AES-256-GCM object at `service-manager/v3/images/<opaque-object-id>.json`. Each image receives an independent random key stored only in the image reference inside the already encrypted Note body; the Note never contains an S3 URL or credentials. A narrow main-process IPC validates, downloads, and decrypts an image before the renderer creates a revocable blob URL. Replacing or removing an image does not automatically delete the now-unreferenced immutable object in this release
    - the manifest is uploaded before the head is published, and the head is the only mutable v3 object. A conditional ETag update of the canonical head provides compare-and-swap publication; if another client wins, the loser reloads the new head and manifest, fetches the newly referenced Note objects it needs, merges, and retries instead of blindly overwriting newer cloud state
    ...
    - legacy settings schemas keep only their non-cloud Endpoint/Bucket/Region/credential configuration, receive a fresh random Sync Key, and discard the old merge base. Legacy SK-encrypted v3 cloud objects and envelopes without a Sync-Key identity are intentionally unsupported and fail closed; this release does not migrate historical cloud data
    - reconciliation is single-flight, local shared-data mutations are serialized against manifest collection/cloud apply, plaintext shared data is bounded to 50 MiB, requests time out after 30 seconds, and active work is aborted and awaited during application shutdown. A late local edit rejects and retries the apply before the local merge base advances. Superseded v3 manifests and Note versions are immutable history; the application does not delete them automatically
...
- `src/main/notesStore.ts`: versioned, bounded local Notes CRUD with serialized atomic per-ID Note/tombstone JSON persistence and recoverable staged whole-set replacement for cloud apply
...
- `src/main/s3DataMerge.ts`: strict shared-data projection, per-ID Notes/tombstone three-way merge, cloud-winning conflict copies/recovery signals, and device-local field overlays
- `src/main/s3SyncV3.ts`: v3 head/manifest/per-Note object contracts, independent Sync-Key AES-GCM envelopes with mandatory key-identity binding, digest-bound references, SigV4 object access, ETag head compare-and-swap, and abortable request bounds
...
- `src/main/s3Sync.ts`: `safeStorage`-protected MinIO/S3 credentials and independent Sync Encryption Key, fresh-key settings-schema upgrade, automatic v3 single-flight reconciliation, same-key Note-reference reuse, independently serialized durable offline/pending intent, encrypted conflict recovery, key rotation, and bounded shutdown ownership
...
- `src/main/uiPreferencesStore.ts`: versioned private device-local UI preferences with bounded Notes editor font-size validation and serialized atomic persistence
- `src/renderer/notesPage.ts`: split-pane CodeMirror 6/Tiptap Notes orchestration with mode conversion confirmation, Name-priority search, tags, icon actions, per-list-row delete, configurable local font sizing, S3-image flow, and debounced live save
- `src/renderer/notesRichTextEditor.ts`: isolated Tiptap adapter with canonical-document serialization, safe S3-image node views, revocable blob-URL ownership, and compact formatting controls
- `src/renderer/settingsDialog.ts`: tabbed S3/Notes settings with a shared save-and-close action, masked credential hydration, non-mutating S3 connectivity testing, automatic sync state, and explicit `Sync Now` retry/check UI
...
- `src/shared/noteRichText.ts`: bounded canonical Tiptap JSON and S3-image-reference validation shared through separate CommonJS main-process and ESM renderer build products
