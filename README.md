...
16. The bottom navigation Settings action provides automatic, cloud-authoritative synchronization through a MinIO-compatible S3 bucket:
    - configure the S3 Endpoint (for example `https://s3.example.com`) and Bucket (for example `service-manager`) separately, followed by Region, Access Key ID, and Secret Access Key. The app owns every object name and internal layout version; legacy settings containing one combined bucket URL are split during migration
    - automatic reconciliation runs after startup, on window focus, after system resume, every 45 seconds as a fallback, and two seconds after a local shared-data change. Offline edits remain in the local JSON stores with a visible pending/offline state and retry on a later trigger. `Sync Now` runs the same reconciler immediately for an explicit check or retry
    - all clients use `service-manager/v2/head.json` as the canonical shared head and store data in immutable `service-manager/v2/revisions/<revision>.json` objects. A conditional ETag update of the head provides compare-and-swap publication; if another client wins, the loser reloads the new head, merges, and retries instead of blindly overwriting it
    ...
    - synchronized data includes durable Host definitions and SSH authentication material, Notes, and shared Proxy mode, selections, Custom Rules, subscription metadata, and retained source subscription. A private-key file path is never uploaded; when needed, its bounded key contents are read into the encrypted revision. Forward auto-start choices, service PIDs, Proxy start-on-launch, Mixed Port, TUN, System Proxy, Kubernetes Context preference, logs, caches, live sessions, runtime state, and S3 configuration/credentials remain device-local or excluded
    ...
    - every immutable revision is signed through AWS SigV4 and encrypted with AES-256-GCM using an HKDF-derived key based on the Secret Access Key. Clients sharing a sync target must therefore use the same SK to decrypt one another's revisions. Plaintext or protected S3 credential material never enters revisions, runtime logs, diagnostics, or error messages
    - reconciliation is single-flight, local shared-data mutations are serialized against snapshot collection/cloud apply, plaintext shared data is bounded to 50 MiB, requests time out after 30 seconds, and active work is aborted and awaited during application shutdown. A late local edit rejects and retries the apply before the local merge base advances. Existing `service-manager/v1/clients/...` objects are retained untouched for compatibility/history but are not automatically restored into the v2 shared head
...
- `src/main/s3DataMerge.ts`: strict v2 shared-data projection, Notes/tombstone three-way merge, cloud-winning conflict copies/recovery signals, and device-local field overlays
- `src/main/s3SyncV2.ts`: Endpoint/Bucket normalization, v2 head and immutable revision objects, AES-GCM envelopes, digest verification, SigV4 GET/PUT, ETag compare-and-swap, and abortable request bounds
- `src/main/s3Sync.ts`: `safeStorage`-protected MinIO/S3 settings and migration, automatic single-flight reconciliation, offline/pending state, encrypted conflict recovery, and bounded shutdown ownership; retained v1 helpers do not import old objects into v2
