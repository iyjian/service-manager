...
6. Service runtime stores the current `pid`; clicking the service name opens a terminal-like log view.
   ...
   - service status itself is auto-refreshed in background (no manual refresh button in list). Each cycle sends one request per Host, locks that Host's requested Services together, waits behind an active start/stop, and uses one combined remote `systemctl list-units`/`show` status exec after a cached systemd capability preflight instead of opening two SSH sessions per Service. Different Hosts run their remote queries concurrently; a transient remote `starting`/`stopping` state remains eligible for the next poll.
...
13. The Hosts header shows total local Service Manager Memory in GB. It aggregates every Electron `app.getAppMetrics()` process and local Mihomo RSS while running, refreshes every five seconds only while Hosts is active, and replaces the former host/tunnel/service count summary. Remote SSH services are excluded, along with system-wide memory. Automatic status polling runs at most four Hosts concurrently. One Host status poll publishes one batch IPC event; preload fans its entries into the existing status callback in the same turn, so service and forwarding status bursts are coalesced to the next animation frame and replace only their exact visible runtime rows. Collapsed Hosts are left untouched, with one coalesced full-render fallback if the expected DOM row is unavailable.
...
15. Notes provides a compact local snippet library:
    - the viewport-contained two-column page uses the full available application width. Its Notion-style Note tree has a `280px` default sidebar that can be dragged or adjusted by keyboard between `240px` and `520px`; that exact device-local width survives window resizing and application restarts while only the editor column changes width. A node with children uses a closed or open folder icon according to its expansion state, while a leaf uses a Note icon, matching the quick folder/Note distinction in Trilium. The tree owns inline root/child creation, collapse controls, subtree deletion, keyboard navigation, and before/inside/after drag-and-drop; clicking a node selects it and also toggles its children, while the dedicated disclosure icon remains available. Both constrained areas scroll internally at smaller window sizes. Rich Text keeps compact horizontal writing padding—24px at ordinary window widths and 12px below the narrow breakpoint—and compensates for its proportional-font metrics so its first visible text line aligns with Markdown while retaining bottom breathing room. Expanded nodes are device-local, while hierarchy and sibling order are durable shared data
    - search ignores collapsed state, ranks exact, prefix, and partial Name matches ahead of lower-priority Tag, Language, and full-content matches, and includes an ancestor breadcrumb for nested results. The renderer maintains invalidatable Note/tree search indexes, reuses cached breadcrumbs until a name or hierarchy changes, and briefly debounces input-driven reranking; ordinary Name typing updates only that visible tree row instead of rebuilding the complete hierarchy
...
- `src/main/notesStore.ts`: versioned, bounded local Notes CRUD with serialized binary-indexed target updates, queue-atomic compare-and-swap saves, atomic per-ID persistence, target-only batch tombstone writes for subtree deletion, and recoverable staged whole-set replacement for cloud apply
...
- `src/main/s3Sync.ts`: `safeStorage`-protected MinIO/S3 credentials and independent Sync Encryption Key, v4-only automatic single-flight reconciliation with privacy-safe phase/item progress, same-key Note/tree-reference reuse, independently serialized durable offline/pending intent, encrypted conflict recovery, key rotation, abort-owned Notes image transfers plus bounded/deduplicated attachment transfers, and bounded shutdown ownership
...
- `src/renderer/renderer.ts`: UI orchestration and DOM event wiring, including animation-frame-coalesced row-local Host runtime status updates with a safe full-render fallback
