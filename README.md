...
The installed application and `pnpm dev` intentionally use the same Electron user-data directory. Only one may run at a time: startup first probes the durable PID/token writer lock (plus Chromium's `SingletonLock` on macOS/Linux), lets Electron serialize current-release contenders, and lets only the winner create or reclaim durable ownership before opening Notes, S3 settings, or another persistent store. Close the installed app before starting `pnpm dev`; a crashed process leaves a reclaimable stale lock.
