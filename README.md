...
15. Notes provides a compact local snippet library:
    - the viewport-contained two-column page uses the full available application width. Its responsive `240–360px` left sidebar is a Notion-style Note tree with inline root/child creation, collapse controls, subtree deletion, keyboard navigation, and before/inside/after drag-and-drop; the selected editor consumes the remaining width and both constrained areas scroll internally at smaller window sizes. Rich Text keeps compact horizontal writing padding—24px at ordinary window widths and 12px below the narrow breakpoint—while retaining its larger vertical breathing room. Expanded nodes are device-local, while hierarchy and sibling order are durable shared data
