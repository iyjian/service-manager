...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - the full-width Kubernetes browser alone uses the available application width rather than the shared centered-content width. Its compact, label-free Context and Namespace selectors share one toolbar row with non-wrapping category and resource controls; there is no UI Cluster category. The Namespace popup remains visible outside that row instead of being clipped, while the category, resource, and secondary control strips keep their own horizontal scrolling when space is constrained. The visible categories are Workloads, Network, Configuration, Storage, and Custom Resources. Custom Resource Definitions are discovered on demand through the read-only ApiextensionsV1 API; choosing Custom Resources immediately opens its Group/Version/Kind discovery select without an intermediate resource-type button
    - the Namespace dropdown loads the active cluster's Namespaces through the read-only API and supports checkbox multi-select between selected Namespaces and the mutually-exclusive `All` menu item; there is no manual Namespace entry. Switching Context invalidates and reloads that list and resets the active scope to All Namespaces. The unclipped Namespace menu stays open during multi-select and closes after an outside pointer press. Nodes and Namespaces remain cluster-scoped regardless of this selection
