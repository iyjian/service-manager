...
- `src/main/kubernetes/kubernetesClient.ts`: main-process-only Kubernetes client adapter for read/list/watch/detail/events, on-demand CRD and Pod→ReplicaSet→Deployment log-target discovery, logs, Pod exec, port forwards, and on-demand related-resource reads
