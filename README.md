...
15. Notes provides a compact local snippet library:
    - the viewport-contained two-column page uses the full available application width. Its responsive `240–360px` left sidebar is a Notion-style Note tree with inline root/child creation, collapse controls, subtree deletion, keyboard navigation, and before/inside/after drag-and-drop; the selected editor consumes the remaining width and both constrained areas scroll internally at smaller window sizes. Expanded nodes are device-local, while hierarchy and sibling order are durable shared data
    ...
    - each note exposes only its Name, Language, and Content editor. Internal Tags remain available to sync, search, and conflict recovery but are not displayed or editable. Markdown remains the default; Markdown, Bash, JavaScript, TypeScript, SQL, JSON, and YAML use CodeMirror 6 with parser-backed highlighting, while Plain Text remains unstyled. Markdown fenced blocks also highlight Bash, JavaScript, TypeScript, SQL, JSON, and YAML. The optional Rich Text language uses a local Tiptap editor and persists a validated canonical Tiptap JSON document instead of HTML. Its Novel-style prose uses the packaged Inter-compatible `STM UI` family with bundled Noto Sans SC for Chinese glyphs. CodeMirror and Tiptap share one device-local Light/Dark preference and default to Light
    - selecting Rich Text opens a compact floating formatter instead of reserving a permanent toolbar row. It includes a complete Text/Heading 1–3/To-do/Bullet/Numbered/Quote/Code block selector, safe link apply/remove, and Bold/Italic/Underline/Strike/Inline Code marks; there is no Ask AI action. Typing `/` opens the matching Novel-style block menu for Text, To-do List, Heading 1/2/3, Bullet List, Numbered List, Quote, Code, and Image. It deliberately omits feedback, YouTube, Twitter, and every AI action. The menu filters as the user types, remains keyboard navigable, and is suppressed inside code blocks. To-do items retain an explicit checked state in the validated canonical JSON model
    - switching a non-empty note between Rich Text and a CodeMirror language requires confirmation because the conversion is intentionally text-oriented and can lose formatting. Rich Text images require a configured S3 target and credentials; without them the image action shows a configuration prompt. Images may be selected from the slash command, pasted, or dropped; uploads accept only PNG, JPEG, and WebP up to 10 MiB, with each dimension limited to 8192px and the decoded image limited to 40 million pixels. An asynchronous paste/drop retains its requested insertion point when that point is still valid
    - edits save automatically after a short debounce, and tree mutations first flush every pending draft so a returned workspace cannot replace unsaved text. Every ordinary save carries the renderer's last persisted Note as a compare-and-swap base. Note/page changes, cloud apply, and window/application close also use the bounded flush path. The Notes page is briefly made non-interactive while a cloud-applied workspace is reloaded; if a new edit lands in the narrow interval after the final pre-apply flush and the cloud has changed or deleted that Note ID, the cloud Note stays canonical and the late draft is preserved as a new `(Conflict)` Note. Every Note ID has one independent versioned `<userData>/notes-v4/<sha256(note.id)>.json` file containing either the active Note or its deletion tombstone, so one edit rewrites only one file. The hierarchy is a separate private atomic `<userData>/notes-tree.json`; local expansion state is isolated in `<userData>/notes-tree-view.json`. This v4 Notes workspace intentionally starts empty and does not read or migrate retired Notes stores
    - the sidebar root and per-row child actions use compact icons. Each tree row owns an icon-only accessible Remove action; confirmation clearly identifies when the complete subtree will be deleted. The main process rechecks the exact confirmed subtree ID set immediately before deletion and rejects a stale confirmation if the tree changed. Copy remains an icon-labelled editor action
...
- `src/main/uiPreferencesStore.ts`: versioned private device-local Notes font-size and shared CodeMirror/Tiptap Light/Dark preferences with serialized atomic persistence
- `src/renderer/notesPage.ts`: full-width split-pane CodeMirror 6/Tiptap Notes orchestration with a searchable drag-and-drop hierarchy, subtree actions, request-generation draft fencing, configurable theme/font sizing, S3-image flow, and debounced live save
- `src/renderer/notesRichTextEditor.ts`: isolated Tiptap adapter with a selection-only Novel-style formatter and keyboard-contained slash block menu, checked task lists, canonical-document serialization, absolute-HTTP(S)-only inert link editing, safe S3-image paste/drop and node views, revocable blob-URL ownership, and disabled remote-image ingestion
