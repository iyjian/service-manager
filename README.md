...
- `src/main/kubernetes/kubeVirtVnc.ts`: strict Running virt-launcher/VMI identity checks and the authenticated `plain.kubevirt.io` WebSocket-to-loopback VNC bridge
...
- `src/main/kubernetes/podInteractions.ts`: bounded single-Pod/Deployment aggregate logs with multi-stream generation fencing, terminal shell fallback/first-output readiness/session lifecycle, and ten-forward ownership
...
- `src/renderer/kubernetesWorkspace.ts`: reusable multi-tab bottom Logs/Shell workspace with Deployment scope switches, post-layout paused-scroll restoration, target-only type-accessible tab visuals, integrated SVG close controls, a bounded pre-bind terminal-output bridge, and exact tab/session lifecycle disposal ownership
