...
- `src/renderer/notesRichTextEditor.ts`: isolated Tiptap adapter with a selection-only Novel-style formatter, keyboard-contained slash block menu and compact left-side current-line six-dot command handle, official resizable TableKit, checked task lists, save-time canonical-document serialization, animation-frame-coalesced editor chrome that skips hidden selection-toolbar state work during ordinary typing, focus-free formatter capability checks, absolute-HTTP(S)-only inert link editing, safe S3-image paste/drop and node views, compact typed attachment cards with capability-gated actions, revocable blob-URL ownership, and disabled remote-image ingestion
