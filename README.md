...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - Context and Namespace controls share one toolbar row. The Namespace dropdown loads the cluster's Namespaces through the read-only API and supports checkbox multi-select between selected Namespaces and the mutually-exclusive All Namespaces shortcut; there is no manual Namespace entry. Nodes and Namespaces remain cluster-scoped regardless of this selection
