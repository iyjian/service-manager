...
14. Kubernetes is a local-kubeconfig, strictly read-only cluster browser for Kubernetes 1.28+:
    ...
    - resource details are full-page, read-only views with Overview, YAML, and on-demand Events. Overview uses compact single-line label/value rows with long values truncated instead of wrapped. Deployment/StatefulSet related Pods and Service Endpoints/EndpointSlices load only after expansion and never add a Watch
    - Pod logs open automatically for the active Pod/container, load 500 initial lines, follow by default, and keep a 2,000-line log cap per viewer. The Logs toolbar keeps search, Follow, and Clear on one row and exposes no manual older-page action. Log streams and terminal sessions close when leaving the Kubernetes page, switching/disconnecting the Context, or exiting the application
